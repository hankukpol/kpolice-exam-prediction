...
export default function HeroFallback({ badge, title, subtitle, isLoggedIn }: HeroFallbackProps) {
  ...
  return (
    <section className="relative overflow-hidden border border-police-900 bg-police-950 text-white shadow-[0_30px_80px_-20px_rgba(0,0,0,0.85)]">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_15%_15%,rgba(58,102,245,0.45),transparent_40%),radial-gradient(circle_at_85%_10%,rgba(92,137,255,0.2),transparent_35%),linear-gradient(175deg,#172145_0%,#22356a_28%,#284ed6_62%,#0f172a_100%)]" />
      <div className="pointer-events-none absolute -bottom-28 left-1/2 h-72 w-[120%] -translate-x-1/2 bg-black/70 blur-3xl" />

      <div className="relative grid gap-6 px-5 py-8 sm:px-8 sm:py-10 lg:grid-cols-[1.25fr_0.75fr] lg:gap-8">
        <div>
          <p className="inline-flex items-center gap-2 border border-white/30 bg-white/10 px-3 py-1 text-xs font-semibold tracking-wide text-white/95">
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            <Link href={primaryHref}>
              <Button className="h-11 bg-white px-6 text-sm font-bold text-black hover:bg-white/90 rounded-none">
                {primaryText}
            ...
            <Link href={isLoggedIn ? "/exam/input" : "/register"}>
              <Button
                variant="outline"
                className="h-11 border-white/50 bg-white/10 px-6 text-sm font-semibold text-white hover:bg-white/20 rounded-none"
              >
        ...
        <div className="border border-white/20 bg-black/35 p-5 backdrop-blur-sm">
          <div className="flex items-center justify-between">
            <p className="text-sm font-semibold text-white/90">서비스 운영 안내</p>
            <Timer className="h-4 w-4 text-white/80" />
          </div>
          <div className="mt-4 grid grid-cols-2 gap-3">
            <article className="border border-white/15 bg-white/10 p-3">
